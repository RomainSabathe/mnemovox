...
class TranscriptionPipeline:
    ...
    def _get_pending_records(self) -> List[tuple]:
        ...
        try:
            records = session.query(Recording).filter_by(transcript_status='pending').all()
    ...
    async def _process_single_record(self, record_id: int, storage_path: str, whisper_model: str):
        """
        ...
        async with self.semaphore:
            ...
            try:
                ...
                if result is None:
                    ...
                    logger.info(f"Transcription completed for record {record_id}: {len(segments)} segments")
                    self._update_record_success(record_id, full_text, segments)
                    
            except Exception as e:
                logger.error(f"Exception during transcription of record {record_id}: {e}")
                self._update_record_error(record_id)
