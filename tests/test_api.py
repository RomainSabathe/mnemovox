...
def test_api_with_real_audio_file_complete_workflow(test_config, test_db_with_records):
    ...
    real_recording = Recording(
        ...
        transcript_segments=[
            ...
            {"start": 1.0, "end": 2.5, "text": "of the audio recording system.", "confidence": 0.87}
        ]
    )
    ...
    assert detail_data["transcript_text"] == "This is a test of the audio recording system."
...
def test_api_transcription_workflow_integration(test_config, test_db_with_records):
    ...
    assert response.status_code == 200
    assert "pending_test.wav" in response.text
    assert ("Transcription in progress" in response.text or "pending" in response.text.lower())
...
def test_api_large_audio_file_handling(test_config, test_db_with_records):
    ...
    many_segments = [
        {"start": i * 2.0, "end": (i + 1) * 2.0, "text": f"Segment {i + 1} content", "confidence": 0.9}
