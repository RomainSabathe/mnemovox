...
def test_ingest_handler_multiple_real_audio_files(test_config, test_db):
    ...
    for i in range(3):
        monitored_file = Path(test_config.monitored_directory) / f"multi_audio_test_{i:02d}.wav"
        shutil.copy2(test_audio_path, monitored_file)
...
def test_ingest_handler_real_audio_file_storage_organization(test_config, test_db):
    ...
    expected_storage_path = f"{today.year}/{today.strftime('%Y-%m-%d')}/{stored_file.name}"
...
def test_ingest_handler_concurrent_file_processing(test_config, test_db):
    ...
    monitored_files = []
    for i in range(5):
        monitored_file = Path(test_config.monitored_directory) / f"concurrent_test_{i:02d}.wav"
        shutil.copy2(test_audio_path, monitored_file)
